...
func (laptopClient *LaptopClient) UploadImage(laptopId string, imagePath string) {
	...
	if err != nil {
		log.Fatal("cannot uplaod image: ", err, stream.RecvMsg(nil))
	}
	...
	if err != nil {
		log.Fatal("cannot create response: ", err, stream.RecvMsg(nil))
	}
...
func (laptopClient *LaptopClient) RateLaptop(laptopIDs []string, scores []float64) error {
	...
	go func() {
		for {
			...
			if err != nil {
				waitResponse <- fmt.Errorf("cannot receive stream response: %v-%v", err, stream.RecvMsg(nil))
				return
			}
